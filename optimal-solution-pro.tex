...
\begin{problem}
  Рассмотрим сбалансированную транспортную задачу, в которой 3 производителя и 3 потребителя. 
  \begin{enumerate}
    \item Сколько базисных и сколько свободных переменных в этой задаче?
    \item Какие переменные можно выбрать базисными, если базисными уже являются $x_{11}$, $x_{21}$, $x_{23}$, $x_{33}$?
    \item Какие переменные можно выбрать базисными, если базисными уже являются $x_{11}$, $x_{12}$, $x_{13}$, $x_{31}$?
    \item Какие переменные можно выбрать базисными, если базисными уже являются $x_{11}$, $x_{13}$, $x_{21}$, $x_{31}$?
  \end{enumerate}
  \begin{sol}
